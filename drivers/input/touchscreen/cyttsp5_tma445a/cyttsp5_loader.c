		...
		struct cyttsp5_touch_firmware *fw)
{
	...
	dev_info(dev, "%s: phone hw ver=0x%02x, fw ver=0x%04x\n", __func__,
		fw_file_ver->hw_ver, get_unaligned_be16(fw_file_ver->fw_ver));
	dev_info(dev, "%s: tsp hw ver=0x%02x, fw ver=0x%04x\n", __func__,
		sti->hw_version, get_unaligned_be16(&sti->fw_versionh));

	/* block fw update for bringup */
#ifdef TEMP_BLOCK_FOR_BRINGUP
	dev_info(dev, "%s: skip fw update for bringup\n", __func__);
	return 0;
#endif

	if (get_unaligned_be16(fw_file_ver->fw_ver) >
...
#endif
{
	...
	if (!upgrade_firmware_from_platform(dev, 0))
		return;
#elif defined(CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE)
