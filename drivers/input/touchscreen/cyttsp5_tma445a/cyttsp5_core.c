...
{
	if (cd->cpdata->power_on_suspend || !CY_WATCHDOG_TIMEOUT)
		return;
...
static void cyttsp5_ta_cb(struct tsp_callbacks *cb, int ta_status)
{
	...
	if (gpio_is_valid(cd->pdata->core_pdata->ta_gpio) && cd->hw_power_state)
...
int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (cd->cpdata->power_on_suspend) {
		...
		if (cd->irq_enabled) {
			cd->irq_enabled = false;
			disable_irq_nosync(cd->irq);
			dev_info(cd->dev, "%s: irq disabled\n", __func__);
		}
		mutex_unlock(&cd->system_lock);
		cd->cpdata->on_off_flag = false;
	} else {
