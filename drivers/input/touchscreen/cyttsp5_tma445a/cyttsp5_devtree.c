		...
		struct device_node *core_node)
{
	...
	pdata->ta_gpio = of_get_named_gpio(core_node, "cy,ta_gpio", 0);
	...
	pdata->pwr_3p3 = of_get_named_gpio(core_node, "cy,pwr_3p3", 0);

	pdata->pwr_1p8 = of_get_named_gpio(core_node, "cy,pwr_1p8", 0);
	...
	pdata->power_on_suspend = of_property_read_bool(core_node, "cy,pwr_on_in_suspend");

	pr_info("[cyttsp5] %s: irq_gpio:%d rst_gpio:%d hid_desc_register:%d\n"
		"[cyttsp5] level_irq_udelay:%d vendor_id:%d product_id:%d\n"
		"[cyttsp5] flags:%d pwr_3p3:%d pwr_1p8:%d ta_gpio:%d power_on_suspend:%d\n",
		__func__,
		pdata->irq_gpio, pdata->rst_gpio, pdata->hid_desc_register,
		pdata->level_irq_udelay, pdata->vendor_id, pdata->product_id,
		pdata->flags, pdata->pwr_3p3, pdata->pwr_1p8,
		pdata->ta_gpio, pdata->power_on_suspend);
