...
static void get_module_vendor(void *device_data)
{
	...
	set_default_result(fdata);
	if (!(gpio_get_value(data->pdata->tsp_en1) &&
				gpio_get_value(data->pdata->tsp_en2))) {
	...
	if (data->pdata->tsp_vendor1 > 0 && data->pdata->tsp_vendor2 > 0) {
		val1 = gpio_get_value(data->pdata->tsp_vendor1);
		val2 = gpio_get_value(data->pdata->tsp_vendor2);
		dev_info(&client->dev,
			"%s: TSP_vendor1: %d[%d] TSP_vendor2: %d[%d] \n", __func__,data->pdata->tsp_vendor1, val1,data->pdata->tsp_vendor2,val2);
		snprintf(buff, sizeof(buff), "%s,%d%d", tostring(OK), val1,val2);
		fdata->cmd_state = CMD_STATUS_OK;
		set_cmd_result(fdata, buff, strnlen(buff, sizeof(buff)));
		return;
	}
...
static int mxt_init_factory(struct mxt_data *data)
{
	...
	if (error) {
        ...
        if (unlikely(error)) {
             printk(KERN_ALERT "Unable to create TSP Symbolic link\n");
        }
