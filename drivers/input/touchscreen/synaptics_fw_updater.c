...
static void synaptics_write_bootload_id(struct synaptics_drv_data *data)
{
	...
	printk("[TSP] fw->f34_reflash_blockdata is %d\n",fw->f34_reflash_blockdata);
...
int synaptics_fw_updater(struct synaptics_drv_data *data, u8 *fw_data)
{
	...
	if (update) {
		...
		if (data->pdata->set_power(data,false))
			data->pdata->hw_reset(data);
		else {
			msleep(100);
			data->pdata->set_power(data,true);
			msleep(100);
		}
