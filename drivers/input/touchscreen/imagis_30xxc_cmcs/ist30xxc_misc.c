...
static u32 *ist30xx_frame_cpbuf;
				...
				const char *buf, size_t size)
{
    ...
    if (direct->cmd == DIRECT_CMD_WRITE) {
        ...
        ist30xx_write_cmd(data->client, DIRECT_ADDR(direct->addr), direct->val);
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    sscanf(buf, "%d %d %d %d %d %d %s", &mode, &delay, &start_ch_rx,
            &stop_ch_rx, &start_ch_tx, &stop_ch_tx, data->rec_file_name);
...
/* sysfs : node */
static DEVICE_ATTR(cp, S_IRUGO, ist30xx_cp_show, NULL);
static DEVICE_ATTR(cp_lower, S_IRUGO, ist30xx_cp_lower_show, NULL);
static DEVICE_ATTR(cp_upper, S_IRUGO, ist30xx_cp_upper_show, NULL);
...
int ist30xx_init_misc_sysfs(struct ist30xx_data *data)
{
	...
	ist30xx_frame_cpbuf = kmalloc(frame_buf_size, GFP_KERNEL);
	if (!ist30xx_frame_buf || !ist30xx_frame_rawbuf ||
			!ist30xx_frame_fltbuf || !ist30xx_frame_cpbuf)
		return -ENOMEM;
