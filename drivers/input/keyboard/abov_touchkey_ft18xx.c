...
#ifdef CONFIG_SEC_A5X_PROJECT
#define TK_FW_PATH_BIN "abov/abov_tk_na5.fw"
#else
#define TK_FW_PATH_BIN "abov/abov_tk_na7.fw"
#endif
...
static void abov_sar_olny_mode(struct abov_tk_info *info, int on)
{
	...
	if(info->dtdata->grip_for_sar != 1){
		dev_err(&client->dev, "%s: unable grip, %d\n", __func__, info->dtdata->grip_for_sar);
		return;
	}
	...
	while(retry>0){
		...
		if (ret < 0) {
			dev_err(&info->client->dev, "%s fail(%d), retry %d\n", __func__, ret, retry);
			retry--;
			msleep(20);
			continue;
		}
...
static irqreturn_t abov_tk_interrupt(int irq, void *dev_id)
{
...
#ifdef FEATURE_GRIP_FOR_SAR		
	if(info->dtdata->grip_for_sar == 1)
		if ((grip_data) && (grip_status!=grip_press)){
			input_report_key(info->input_dev,
	...
	if(info->dtdata->grip_for_sar == 1)
		if ((grip_data) && (grip_status!=grip_press)){
			dev_notice(&client->dev, "%s%x \n",
				grip_press ? "grip P " : "grip R ",	buf);
			grip_status = grip_press;
		}
#endif
		 ...
		 size_t count)
{
	...
	ret = abov_tk_i2c_write(info->client, CMD_SAR_ENABLE, &cmd, 1);
	if (ret < 0) {
		dev_err(&info->client->dev, "%s fail(%d)\n", __func__, ret);
		return count;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	if(info->dtdata->grip_for_sar != 1){
		dev_err(&info->client->dev, "%s: unable grip, %d\n", __func__, info->dtdata->grip_for_sar);
		return sprintf(buf, "%d\n", 0);
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	info->grip_raw2 = (r_buf[2] << 8) | r_buf[3];
...
static int abov_tk_i2c_read_checksum(struct abov_tk_info *info)
{
	...
	dev_info(&info->client->dev, "%s: ret:%d [%X][%X][%X][%X][%X][%X]\n",
			...
			struct abov_touchkey_devicetree_data *dtdata)
{
	...
	dtdata->grip_for_sar = of_property_read_bool(np, "abov,grip_for_sar");
	if(dtdata->vdd_io_alwayson < 0){
		dev_err(dev, "unable to get grip_for_sar\n");
		dtdata->grip_for_sar = 0;
	}
	...
	dev_info(dev, "%s: gpio_en:%d, gpio_int:%d, gpio_scl:%d,"
		" gpio_sda:%d, gpio_led_en:%d, vdd_io:%d, grip:%d\n",
...
static int abov_tk_input_open(struct input_dev *dev)
{
	...
	if(info->dtdata->grip_for_sar == 1){
		...
		dev_info(&info->client->dev, "%s:  enter normal mode in open()", __func__);
...
static void abov_tk_input_close(struct input_dev *dev)
{
	...
	if(info->dtdata->grip_for_sar == 1){
		...
		dev_info(&info->client->dev, "%s:  enter sar only mode in close()", __func__);
