 ...
 *
*/
#include "ss_dsi_panel_EA8061_AMS549BU19_ID400418.h"
#include "ss_dsi_mdnie_EA8061_AMS549BU19_ID400418.h"

char id;
char elvss_buffer1[1];
...
static struct dsi_panel_cmds * mdss_elvss(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (id >= 0x04) {
		...
		if (vdd->temperature > 0) {
			...
			switch(cd_index) {
				case 0 ... 19:	// between 5 nit and 27 nit
						vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x0C; /* ELVSS */
						break;
				case 20:	//29 nit
						vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x0B; /* ELVSS */
						break;
				default:
					break;
			}
			...
			switch(cd_index) {
				case 0 ... 14:	// between 5 nit and 21 nit
					if(vdd->temperature <= -20 )
				...
				case 15:	//21 nit
					if(vdd->temperature <= -20 )
						vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x15; /* ELVSS */
					else
						vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x12; /* ELVSS */
					break;
				case 16:	//22 nit
					if(vdd->temperature <= -20 )
						vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x13; /* ELVSS */
					else
						vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x12; /* ELVSS */
					break;
				case 17:	// 24 nit
					vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x11; /* ELVSS */
					break;
				case 18:	// 25 nit
					vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x0F; /* ELVSS */
					break;
				case 19:	// 27 nit
					vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x0D; /* ELVSS */
					break;
				case 20:	// 29 nit
					vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = 0x0B; /* ELVSS */
					break;
		...
		if(cd_index >= 21) {	// over 30 nits
			...
			vdd->dtsi_data[ctrl->ndx].gamma_tx_cmds[vdd->panel_revision].cmds[5].payload[1] = elvss_buffer1[0];
		}
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_EA8061_ID;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_EA8061_ID400418_HD";
