...
static int searching_function(long long candela, int *index, int gamma_curve)
{
	...
	*index = -1;
	...
	for (cnt = 0; cnt < (GRAY_SCALE_MAX - 1); cnt++) {
		if (gamma_curve == GAMMA_CURVE_1P95) {
			delta_1 = candela - curve_1p95_300[cnt];
			delta_2 = candela - curve_1p95_300[cnt + 1];
		} else if (gamma_curve == GAMMA_CURVE_2P0) {
			delta_1 = candela - curve_2p0_300[cnt];
			delta_2 = candela - curve_2p0_300[cnt + 1];
		} else if (gamma_curve == GAMMA_CURVE_2P05) {
			delta_1 = candela - curve_2p05_300[cnt];
			delta_2 = candela - curve_2p05_300[cnt + 1];
		} else if (gamma_curve == GAMMA_CURVE_2P1) {
			delta_1 = candela - curve_2p1_300[cnt];
			delta_2 = candela - curve_2p1_300[cnt + 1];
		} else if (gamma_curve == GAMMA_CURVE_2P12) {
			delta_1 = candela - curve_2p12_300[cnt];
			delta_2 = candela - curve_2p12_300[cnt + 1];
		} else if (gamma_curve == GAMMA_CURVE_2P2) {
			delta_1 = candela - curve_2p2_300[cnt];
			delta_2 = candela - curve_2p2_300[cnt + 1];
		} else {
			delta_1 = candela - curve_2p2_300[cnt];
			delta_2 = candela - curve_2p2_300[cnt + 1];
		}
...
static void gamma_init(struct SMART_DIM *pSmart, char *str, int size)
{
	...
	} else if (pSmart->brightness_level >= 16 && pSmart->brightness_level < 25) {
		for (cnt = 0; cnt < TABLE_MAX; cnt++) {
			point_index = INFLECTION_VOLTAGE_ARRAY[cnt+1];
			temp_cal_data =
	...
	} else if (pSmart->brightness_level >= 25 && pSmart->brightness_level < 41) {
		for (cnt = 0; cnt < TABLE_MAX; cnt++) {
			point_index = INFLECTION_VOLTAGE_ARRAY[cnt+1];
			temp_cal_data =
	...
	} else if (pSmart->brightness_level >= 41 && pSmart->brightness_level < 111) {
		for (cnt = 0; cnt < TABLE_MAX; cnt++) {
			point_index = INFLECTION_VOLTAGE_ARRAY[cnt+1];
			temp_cal_data =
	...
	} else if (pSmart->brightness_level >= 111 && pSmart->brightness_level < 183) {
		for (cnt = 0; cnt < TABLE_MAX; cnt++) {
			point_index = INFLECTION_VOLTAGE_ARRAY[cnt+1];
			temp_cal_data =
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	...
	psmart->vregout_voltage = VREG0_REF_6P1;
