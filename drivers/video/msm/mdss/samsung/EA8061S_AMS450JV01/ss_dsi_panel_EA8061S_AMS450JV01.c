 ...
 *
*/
#include "ss_dsi_panel_EA8061S_AMS450JV01.h"
#include "ss_dsi_mdnie_EA8061S_AMS450JV01.h"
...
static int mdss_mdnie_read(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	/* Read mtp (0xA1 : 1~4 th) for ddi id */
	if (vdd->dtsi_data[ctrl->ndx].mdnie_read_rx_cmds[vdd->panel_revision].cmd_cnt) {
		...
		pr_err("%s DSI%d : X-%d Y-%d \n", __func__, ctrl->ndx, vdd->mdnie_x[ctrl->ndx], vdd->mdnie_y[ctrl->ndx]);
	} else {
		pr_err("%s DSI%d error", __func__, ctrl->ndx);
		return false;
	}
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_EA8061S_AMS450JV01;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_EA8061S_AMS450JV01_WVGA";

	vdd->panel_name = mdss_mdp_panel + 8;
	pr_info("[mdss] %s : %s\n", __func__, vdd->panel_name);

	strlcpy(vdd->panel_name, "ss_dsi_panel_EA8061S_AMS450JV01_WVGA", 256);
