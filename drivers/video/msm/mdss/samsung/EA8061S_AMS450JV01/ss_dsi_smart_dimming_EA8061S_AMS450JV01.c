 ...
 *
*/
#include "ss_dsi_smart_dimming_EA8061S_AMS450JV01.h"
...
static void print_RGB_offset(struct SMART_DIM *pSmart)
{
	...
	pr_info("DSI0 %s MTP Offset V23 R:%d G:%d B:%d\n", __func__,
	...
	pr_info("DSI0 %s MTP Offset V35 R:%d G:%d B:%d\n", __func__,
	...
	pr_info("DSI0 %s MTP Offset V51 R:%d G:%d B:%d\n", __func__,
	...
	pr_info("DSI0 %s MTP Offset V87 R:%d G:%d B:%d\n", __func__,
	...
	pr_info("DSI0 %s MTP Offset V151 R:%d G:%d B:%d\n", __func__,
	...
	pr_info("DSI0 %s MTP Offset V203 R:%d G:%d B:%d\n", __func__,
	...
	pr_info("DSI0 %s MTP Offset V255 R:%d G:%d B:%d\n", __func__,
...
static void(*Make_hexa[TABLE_MAX])(int*, struct SMART_DIM*, int*) = {
	v255_hexa,
	v203_hexa,
	v151_hexa,
	v87_hexa,
...
static void gamma_init(struct SMART_DIM *pSmart, char *str, int size)
{
	...
	*/
	for (cnt = 1; cnt < TABLE_MAX; cnt++) {
		...
		bl_index[TABLE_MAX - cnt] += get_gradation_offset(table_index, cnt - 1);
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	...
	for (lux_loop = 0; lux_loop < psmart->lux_table_max; lux_loop++) {
		psmart->brightness_level = psmart->plux_table[lux_loop];	/* To set brightness value */
		psmart->gen_table[lux_loop].lux = psmart->plux_table[lux_loop];	/* To make lux table index*/

#if defined(AID_OPERATION)
		gamma_init(psmart, (char *)(&(psmart->gen_table[lux_loop].gamma_setting)), GAMMA_SET_MAX);
#else
		pure_gamma_init(psmart, (char *)(&(psmart->gen_table[lux_loop].gamma_setting)), GAMMA_SET_MAX);
#endif
	}
...
struct smartdim_conf *smart_get_conf_EA8061S_AMS450JV01(void)
{
