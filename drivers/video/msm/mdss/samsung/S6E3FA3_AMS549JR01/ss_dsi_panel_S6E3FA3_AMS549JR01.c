 ...
 *
*/
#include "ss_dsi_panel_S6E3FA3_AMS549JR01.h"
#include "ss_dsi_mdnie_S6E3FA3_AMS549JR01.h"
...
static int mdss_cell_id_read(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (vdd->dtsi_data[ctrl->ndx].cell_id_rx_cmds[vdd->panel_revision].cmd_cnt) {
		...
		/* For AMS549JR01 exceptionally, MDNIE X&Y value(4bytes) should be get from A1h register read value */
	...
	if (vdd->temperature > 0) {
		if (candela_value == 2)	/* 2 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x10;
		else if (candela_value < 14) /* 3 ~ 14 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x0F;
		else if (candela_value == 14) /* 3 ~ 14 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
		...
		if (candela_value == 2)	/* 2 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x19;
		else if (candela_value < 5) /* 3 ~ 4 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x17;
		else if (candela_value == 5) /* 5 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x15;
		else if (candela_value < 8) /* 6~7 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x13;
		else if (candela_value < 10) /* 8~9 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x12;
		else if (candela_value < 12) /* 10~11 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x11;
		else if (candela_value == 12) /* 12 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x10;
		else if (candela_value == 13) /* 12 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x0F;
		else if (candela_value == 14) /* 14 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
				= 0x0E;
		else /* 15 ~ 420 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1]
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3FA3_AMS549JR01;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3FA3_AMS549JR01_FHD";
