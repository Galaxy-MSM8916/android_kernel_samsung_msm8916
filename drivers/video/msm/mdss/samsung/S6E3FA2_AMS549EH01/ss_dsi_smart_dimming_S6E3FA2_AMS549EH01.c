 ...
 *
*/
#include "ss_dsi_smart_dimming_S6E3FA2_AMS549EH01.h"
...
static void gamma_init_revI(struct SMART_DIM *pSmart, char *str, int size)
{
	...
	if (pSmart->brightness_level >= 249) {
		...
		if (pSmart->brightness_level == 249)
			...
			bl_level = 360;
	} else if ((pSmart->brightness_level <= 248) &&
					(pSmart->brightness_level >= 162)) {
		bl_level = 262;
	} else if ((pSmart->brightness_level < 161) &&
	...
	*/
	for (cnt = 0; cnt < RGB_COMPENSATION; cnt++) {
		...
		if (cnt < 3) {
			...
			gamma_setting[cnt+3] += rgb_offset_revI[table_index][cnt];
		}
	}
...
struct smartdim_conf *smart_get_conf_S6E3FA2_AMS549EH01(void) {
