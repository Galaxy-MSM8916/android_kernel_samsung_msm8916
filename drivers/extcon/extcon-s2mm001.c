...
static void s2mm001_muic_handle_attach(struct s2mm001_muic_data *muic_data,
			enum extcon_cable_name new_dev, int adc, u8 vbvolt);
...
{
	struct s2mm001_muic_data *muic_data = dev_get_drvdata(dev);
	enum extcon_cable_name new_dev = EXTCON_USB;
...
static int attach_charger(struct s2mm001_muic_data *muic_data,
			enum extcon_cable_name new_dev)
{
	int ret = 0;
...
static int detach_charger(struct s2mm001_muic_data *muic_data)
{
	...
	muic_data->attached_dev = EXTCON_NONE;
...
static int attach_deskdock(struct s2mm001_muic_data *muic_data,
			enum extcon_cable_name new_dev, u8 vbvolt)
{
	int ret = 0;
...
static int attach_audiodock(struct s2mm001_muic_data *muic_data,
			enum extcon_cable_name new_dev, u8 vbus)
{
	int ret = 0;
				...
				enum extcon_cable_name new_dev)
{
	...
	pr_info("%s:%s JIG UART BOOT-OFF(%d)\n",
		MUIC_DEV_NAME, __func__, new_dev);
				...
				u8 vbvolt)
{
	...
	if (muic_data->attached_dev == EXTCON_JIG_USBOFF) {
	...
	ret = attach_usb_util(muic_data, EXTCON_JIG_USBOFF);
	if (ret)
		return ret;
				...
				u8 vbvolt)
{
	...
	if (muic_data->attached_dev == EXTCON_JIG_USBON) {
	...
	ret = attach_usb_util(muic_data, EXTCON_JIG_USBON);
	if (ret)
		return ret;
...
static void check_attached_device(struct s2mm001_muic_data *muic_data, int attached)
//static void _detected(struct max77849_muic_info *info, u32 new_state)
{
	...
	if (attached)
		new_state = BIT(muic_data->attached_dev);
	else
		new_state = 0;

	current_state = muic_data->edev->state;
	changed_state = current_state ^ new_state;

	pr_info("state: cur=0x%x, new=0x%x, changed=0x%x\n",
			current_state, new_state, changed_state);

	if((current_state != 0x0) && (new_state == BIT(EXTCON_USB))) {
		pr_info("Force detach.  cur=0x%x, new=0x%x, changed=0x%x\n",
				current_state, new_state, changed_state);
		new_state = 0;
		changed_state = current_state ^ new_state;
	}
	...
	for (index = 0; index < SUPPORTED_CABLE_MAX; index++) {
		...
		ret = extcon_set_cable_state(muic_data->edev,
				extcon_cable_name[index], attach);
		if (unlikely(ret < 0))
			pr_err("fail to notify %s(%d), %d, ret=%d\n",
				extcon_cable_name[index], index, attach, ret);
	}
}
#if 0
static void _detected(struct s2mm001_muic_data *muic_data, int attach)
{
	int ret;

	if ((!attach) && (muic_data->attached_dev == EXTCON_NONE)) {
		pr_info("%s, attached cable is nothing\n", __func__);

		return;
	}

	ret = extcon_set_cable_state(muic_data->edev,
			extcon_cable_name[muic_data->attached_dev], attach);
	if (unlikely(ret < 0))
		pr_err("fail to notify %s(%d), %d, ret=%d\n",
			extcon_cable_name[muic_data->attached_dev],
			muic_data->attached_dev, attach, ret);
}
#endif
static void s2mm001_muic_handle_attach(struct s2mm001_muic_data *muic_data,
			enum extcon_cable_name new_dev, int adc, u8 vbvolt)
{
	...
	if (noti)
		check_attached_device(muic_data, 0);
	...
	case EXTCON_JIG_UARTOFF:
		if (vbvolt) {
			if (muic_data->is_otg_test)
				new_dev = EXTCON_USB_HOST_5V;
			else
				new_dev = EXTCON_JIG_UARTOFF_VB;
		} else
			new_dev = EXTCON_JIG_UARTOFF;
	...
	if (noti)
		check_attached_device(muic_data, 1);
}
...
{
	struct i2c_client *i2c = muic_data->i2c;
	enum extcon_cable_name new_dev = EXTCON_UNKNOWN;
	...
	case DEV_TYPE1_CDP:
		...
		new_dev = EXTCON_CHARGE_DOWNSTREAM;
	...
	case DEV_TYPE1_USB:
		...
		if (muic_data->sdp_skip) {
			...
			pr_info("%s : TIMEOUT CHARGER DETECTED\n",
						    MUIC_DEV_NAME);
		} else {
			new_dev = EXTCON_USB;
	...
	if (new_dev == EXTCON_UNKNOWN) {
		...
		case ADC_JIG_UART_ON:
			/* This is the mode to wake up device
						during factory mode. */
			if (new_dev != EXTCON_JIG_UARTON
					&& muic_data->is_factory_start) {
				intr = MUIC_INTR_ATTACH;
				new_dev = EXTCON_JIG_UARTON;
				...
				const struct i2c_device_id *id)
{
	...
	muic_data->edev = devm_kzalloc(muic_data->dev, sizeof(struct extcon_dev),
			GFP_KERNEL);
	if (!muic_data->edev) {
		pr_err("failed to allocate memory for extcon\n");
		ret = -ENOMEM;
		goto fail_init_gpio;
	}

	muic_data->edev->name=EXTCON_DEV_NAME;
	muic_data->edev->supported_cable = extcon_cable_name;

	ret = extcon_dev_register(muic_data->edev, NULL);
	if (ret) {
		pr_err("failed to register extcon device\n");
		goto fail_init_gpio;
	}
	muic_data->pdata->usb_path = MUIC_PATH_USB_AP;
	...
	muic_data->attached_dev = EXTCON_UNKNOWN;
	muic_data->is_usb_ready = false;
	muic_data->sdp_skip = false;

	/* create sysfs group */
	ret = sysfs_create_group(&switch_dev->kobj, &s2mm001_muic_group);
	if (ret) {
		pr_err("%s: failed to create s2mm001 muic attribute group\n",
			__func__);
		goto fail;
	}
	dev_set_drvdata(switch_dev, muic_data);
	...
	schedule_delayed_work(&muic_data->init_work, msecs_to_jiffies(5000));
...
fail:
	sysfs_remove_group(&switch_dev->kobj, &s2mm001_muic_group);
...
static int s2mm001_muic_suspend(struct device *dev)
{
	...
	//ret = s2mm001_i2c_write_byte(client, 0x2B, 0x80);
	muic_data->mansw = s2mm001_i2c_read_byte(muic_data->i2c, S2MM001_MUIC_REG_MANSW1);
	s2mm001_muic_jig_on(muic_data);
	set_com_sw(muic_data, MANSW1_OPEN_WITH_V_BUS);
	ret = s2mm001_i2c_read_byte(muic_data->i2c, S2MM001_MUIC_REG_CTRL);
	if (ret < 0)
		pr_err("%s: err REG_CTRL (%d)\n", __func__, ret);
	muic_data->mode = ret & CTRL_MANUAL_SW_MASK;
	set_manual_sw(muic_data, false);
	return 0;
}

static int s2mm001_muic_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct s2mm001_muic_data *muic_data = i2c_get_clientdata(client);
	int ret;
	...
	pr_info("%s: resume\n", __func__);
	if (muic_data->mode == CTRL_MANUAL_SW_MASK) {
		set_manual_sw(muic_data, true);
		s2mm001_muic_jig_on(muic_data);
	} else {
		ret = s2mm001_i2c_write_byte(client, S2MM001_MUIC_REG_MANSW1,
			muic_data->mansw);
		if (ret < 0)
			dev_err(&client->dev, "%s: write REG_MANSW1 err %d\n",
				__func__, ret);
	}
//	ret = s2mm001_i2c_write_byte(client, 0x2B, 0x00);

	return 0;
}

static const struct dev_pm_ops s2mm001_muic_pm_ops = {
