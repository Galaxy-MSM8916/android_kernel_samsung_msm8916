	...
	union power_supply_propval *val)
{
	...
	if ((value.intval != POWER_SUPPLY_TYPE_BATTERY) &&
		...
		if ((curr < capacity_threshold) && (curr > topoff)) {
			...
			} else if (fuelgauge->standard_capacity < 1000) {
			     ...
			     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL:
		val->intval = max77843_get_fuelgauge_value(fuelgauge, FG_FULLCAP) * 100 / fuelgauge->battery_data->Capacity;
		break;
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		val->intval = fuelgauge->pdata->capacity_max;
		break;
	default:
		return -EINVAL;
	}
			     ...
			     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		pr_info("%s: capacity_max changed, %d -> %d\n",
			__func__, fuelgauge->pdata->capacity_max, val->intval);
		fuelgauge->pdata->capacity_max = val->intval;
		break;
	default:
		return -EINVAL;
	}
...
static int max77843_fuelgauge_parse_dt(struct max77843_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		if (ret > 0) {
			pdata->jig_irq = gpio_to_irq(ret);
			pdata->jig_irq_attr = IRQF_TRIGGER_RISING;
			pr_info("%s reading jig_irq = %d\n",
		...
		pr_info("%s fg_irq: %d, capacity_max: %d\n"
			...
			pdata->repeated_fuelalert, pdata->jig_irq);
	}
