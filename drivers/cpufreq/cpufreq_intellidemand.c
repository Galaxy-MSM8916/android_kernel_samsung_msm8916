...
#define DEF_STEP_UP_EARLY_HISPEED		(1209600)
...
#define SUP_SLOW_UP_FREQUENCY 		(1209600)
...
static inline cputime64_t _get_cpu_idle_time(unsigned int cpu, cputime64_t *wall)
{
                                      ...
                                      const char *buf, size_t count)
{
        ...
        for_each_online_cpu(j) {
                struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(id_cpu_dbs_info, j);
                dbs_info->prev_cpu_idle = _get_cpu_idle_time(j,
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
		...
		j_dbs_info = &per_cpu(id_cpu_dbs_info, j);
                ...
                cur_idle_time = _get_cpu_idle_time(j, &cur_wall_time);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	this_dbs_info = &per_cpu(id_cpu_dbs_info, cpu);
        ...
        if (policy->cur < policy->max) {
                ...
                this_dbs_info->prev_cpu_idle = _get_cpu_idle_time(cpu,
                                   ...
                                   unsigned int event)
{
	...
	this_dbs_info = &per_cpu(id_cpu_dbs_info, cpu);
        ...
        case CPUFREQ_GOV_START:
			...
			j_dbs_info->prev_cpu_idle = _get_cpu_idle_time(j,
...
static int cpufreq_gov_dbs_up_task(void *data)
{
		...
		this_dbs_info = &per_cpu(id_cpu_dbs_info, cpu);
                ...
                if (policy->cur < input_event_min_freq) {
                        ...
                        this_dbs_info->prev_cpu_idle = _get_cpu_idle_time(cpu, &this_dbs_info->prev_cpu_wall);
                }
