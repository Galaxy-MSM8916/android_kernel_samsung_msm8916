...
#define DEFAULT_SUSPEND_IDEAL_FREQ 200000
...
#define DEFAULT_TOUCH_POKE_FREQ 1209600
#define DEFAULT_BOOST_FREQ 1478400
...
static inline void smartmax_dbs_timer_init(struct smartmax_info_s *this_smartmax) {
	...
	INIT_DEFERRABLE_WORK(&this_smartmax->work, smartmax_do_dbs_timer);
...
static void inline cpufreq_smartmax_calc_load(int j)
{
	...
	cur_idle_time = _get_cpu_idle_time(j, &cur_wall_time, 0);
...
static void update_idle_time(bool online) {
	...
	for_each_possible_cpu(j)
	{
		...
		j_this_smartmax->prev_cpu_idle = _get_cpu_idle_time(j,
...
static int cpufreq_smartmax_boost_task(void *data) {
	...
	while (1) {
        ...
        this_smartmax->prev_cpu_idle = _get_cpu_idle_time(0,
		...
		for_each_online_cpu(cpu){
			...
			if (policy->cur < cur_boost_freq) {
				...
				this_smartmax->prev_cpu_idle = _get_cpu_idle_time(cpu, &this_smartmax->prev_cpu_wall, 0);
			}
			mutex_unlock(&this_smartmax->timer_mutex);
...
static int cpufreq_governor_smartmax(struct cpufreq_policy *new_policy,
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			...
			rc = sysfs_create_group(get_governor_parent_kobj(new_policy),
					&smartmax_attr_group);
			if (rc) {
				dbs_enable--;
				if (!have_governor_per_policy()) {
	...
	case CPUFREQ_GOV_STOP:
		...
		if (!dbs_enable){
			...
			sysfs_remove_group(get_governor_parent_kobj(new_policy),
				&smartmax_attr_group);
