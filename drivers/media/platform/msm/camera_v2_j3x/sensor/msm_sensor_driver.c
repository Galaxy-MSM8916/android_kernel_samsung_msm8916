...
extern struct device *cam_dev_front;
...
static ssize_t check_i2c_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
    int rc = 0, i = 0;
    char check_data_type;
    uint16_t read_data, i2c_page, i2c_addr;
    struct msm_sensor_ctrl_t            *s_ctrl = NULL;
    int check_size = strlen(buf);
    unsigned long i2c_value;
    char i2c_page_addr[9] = {0};

    pr_err("%s : E", __FUNCTION__);

    if (check_size == 10) { // check 9 byte because of string
        check_data_type = buf[0]; /* data type char or short */
        for (i = 0; i < check_size - 1; i++)
            i2c_page_addr[i] = buf[i+1];

        i2c_value = simple_strtoul(i2c_page_addr, NULL, 16);
        s_ctrl = g_sctrl[1];
        if (!s_ctrl)
            pr_err("failed: s_ctrl %p for camera_id", s_ctrl);
        else
            pr_err("get sctrl");

        //        check_data_type = (int)(i2c_value & 0xF00000000) >> 32);
        i2c_page = (uint16_t)((i2c_value & 0xFFFF0000) >> 16);
        i2c_addr = (uint16_t)(i2c_value & 0x0000FFFF);
        pr_err("[check i2c] buf : %s, check_data_type %c, i2c_page 0x%x,i2c_addr 0x%x\n",
               i2c_page_addr, check_data_type, i2c_page, i2c_addr);

        rc = msm_camera_cci_i2c_write(s_ctrl->sensor_i2c_client, 0xFCFC,
                                      i2c_page, 0x02);
        if (rc < 0) {
            pr_err("%s:%d] page setting failed", __FUNCTION__, __LINE__);
            return rc;
        }

        if (check_data_type == '1') { /* check data type char or short */
            rc = msm_camera_cci_i2c_read(s_ctrl->sensor_i2c_client, i2c_addr,
                                         &read_data, 0x01);
        } else {
            rc = msm_camera_cci_i2c_read(s_ctrl->sensor_i2c_client, i2c_addr,
                                         &read_data, 0x02);
        ...
        if (rc < 0) {
            pr_err("[%s:%d] i2c read failed", __FUNCTION__, __LINE__);
        }

        pr_err("[check i2c] read data 0x%04x\n", read_data);
    } else
        pr_err("[%s:%d] size(%d) is invalid", __FUNCTION__, __LINE__, check_size);

    return size;
}

static DEVICE_ATTR(check_i2c, S_IWUSR|S_IWGRP|S_IROTH,
	NULL, check_i2c_store);
...
//		goto FREE_CAMERA_INFO;
	...
	if (!IS_ERR(camera_class) && !IS_ERR(cam_dev_front) &&
        (slave_info->camera_id == 1)) { //for IRIS
        if (device_create_file(cam_dev_front, &dev_attr_check_i2c) < 0) {
            pr_err("failed to create device file, %s\n",
                   dev_attr_check_i2c.attr.name);
        }
    } else
        pr_err("create check camera class!\n");
...
static int __init msm_sensor_driver_init(void)
{
	...
	if (rc >= 0) {
		CDBG("probe success");
#if defined(CONFIG_MACH_GOYAIRIS3G_SWA_OPEN)
