...
static void set_vibrator(struct bldc_vib *vib, int on)
{
	pr_info("[VIB] %s, value[%d]\n",__func__, on);
	gpio_direction_output(vib->motor_en, on);
	gpio_direction_output(vib->motor_control, on);
}

static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	struct bldc_vib *vib = container_of(dev, struct bldc_vib,
					 timed_dev);
...
{
	struct bldc_vib *vib = container_of(work, struct bldc_vib,
					 work);
...
{
	struct bldc_vib *vib = container_of(timer, struct bldc_vib,
							 vib_timer);
...
static int bldc_vibrator_probe(struct platform_device *pdev)
{
	...
	vib->motor_en = of_get_named_gpio(pdev->dev.of_node, "bldc,vib_en", 0);
	pr_info("[VIB] %s motor_en %d\n",__func__, vib->motor_en);

	vib->motor_control = of_get_named_gpio(pdev->dev.of_node, "bldc,vib_control", 0);
	pr_info("[VIB] %s motor_control %d\n",__func__, vib->motor_control);
	...
	if (!gpio_is_valid(vib->motor_control)) {
		pr_err("%s:%d, reset gpio not specified\n",
				__func__, __LINE__);
		return -EINVAL;
	}
	if (gpio_request(vib->motor_control, "motor_control")) {
		pr_err("%s:%d, request not specified\n",
				__func__, __LINE__);
		return -EINVAL;
	}
